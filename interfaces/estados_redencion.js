angular.module('estadosRedencionApp', []).controller('estadosRedencionController', function ($scope, $http) {
    ...
    $scope.SumarioFoliosMasivos = function()
    {
        ...
        angular.forEach(listado, function(folio){
            ...
            if( informacion_folio.length != 4 ) { folio_correcto = false; }
            else if( !$.isNumeric( informacion_folio[0]) ) { folio_correcto = false; }
            else if( !$.isNumeric( informacion_folio[1]) ) { folio_correcto = false; }
            else if( informacion_folio[2].length > 100 ) { folio_correcto = false; }
